...
### 11.4B 直接画图对比：新 VAE vs DA-VAE

下面用同一个 `1024×1024` 输入、同一个 `patch_size=2` DiT 入口来画。两者最后进入 DiT 的 sequence token 数相同，都是 `32×32 = 1024`；区别是 latent grid 的通道数和通道语义。
...
└──────────────────────────────────────────────────────────────────────────────┘
                 ...
                 ▼                                           ▼
┌──────────────────────────────────────┐   ┌──────────────────────────────────────┐
│ DA-VAE 16× (SD3/Flux VAE + da=2)     │   │ 新 VAE 16×-64ch                     │
├──────────────────────────────────────┤   ├──────────────────────────────────────┤
│ 原 VAE Encoder                       │   │ 独立 Encoder                        │
│ 1024 → 512 → 256 → 128               │   │ 1024 → 512 → 256 → 128 → 64         │
│                                      │   │                                      │
│ pre-conv feature                     │   │ bottleneck feature                  │
│ [B, 512, 128, 128]                   │   │ [B, 512, 64, 64]                    │
│                                      │   │                                      │
│ DCDownBlock2d                        │   │ Encode_conv_out_shortcut            │
│ 额外 2× 空间压缩                     │   │ 只做通道压缩，不再降空间             │
│                                      │   │                                      │
│ moments                              │   │ moments                              │
│ full:   [B, 64, 64, 64]              │   │ [B, 128, 64, 64]                    │
│ detail: [B, 32, 64, 64]              │   │                                      │
│                                      │   │                                      │
│ sample                               │   │ sample                               │
│ full:   z_full   [B, 32, 64, 64]     │   │ z_new [B, 64, 64, 64]               │
│ detail: z_detail [B, 16, 64, 64]     │   │                                      │
│         + z_base [B,16,64,64]        │   │                                      │
│         = z_total [B,32,64,64]       │   │                                      │
└──────────────────────────────────────┘   └──────────────────────────────────────┘
                 │                                           │
                 ▼                                           ▼
┌──────────────────────────────────────┐   ┌──────────────────────────────────────┐
│ DiT PatchEmbed                       │   │ DiT PatchEmbed                       │
│ patch_size = 2                       │   │ patch_size = 2                       │
│                                      │   │                                      │
│ [B, 32, 64, 64]                      │   │ [B, 64, 64, 64]                      │
│    4096 latent cells                 │   │    4096 latent cells                 │
│    → 32×32 = 1024 DiT tokens         │   │    → 32×32 = 1024 DiT tokens         │
│                                      │   │                                      │
│ 每个 patch token 看 2×2×32 数值       │   │ 每个 patch token 看 2×2×64 数值       │
│ = 128 latent scalars                 │   │ = 256 latent scalars                 │
└──────────────────────────────────────┘   └──────────────────────────────────────┘
```
...
```
DA-VAE:
1024 image → 8× VAE feature [512,128,128]
           → DA 额外 2× → latent [32,64,64]
           → PatchEmbed p=2 → 1024 DiT tokens

新 VAE:
1024 image → 4 次 2× downsample → latent [64,64,64]
           → PatchEmbed p=2 → 1024 DiT tokens
```
...
```
DA-VAE detail/diff latent [B, 32, 64, 64]
┌──────────────────────────────┬──────────────────────────────┐
│ base channels: 16             │ detail/diff channels: 16      │
│ 来自原 VAE / teacher latent   │ 来自新增 DA encoder            │
│ 对预训练 DiT 更熟悉           │ zero-init branch 慢慢学习使用  │
└──────────────────────────────┴──────────────────────────────┘

新 VAE latent [B, 64, 64, 64]
┌──────────────────────────────────────────────────────────────┐
│ 64 个均匀 latent channels                                    │
│ 没有显式 base/detail 切分；DiT patch embed 需要整体适配这 64ch │
└──────────────────────────────────────────────────────────────┘
```
...
```
DA-VAE decoder:
[B,32,64,64] → DCUpBlock2d 2× → [B,512,128,128]
             → 跳过原 decoder.conv_in，接回原 VAE Decoder → image

新 VAE decoder:
[B,64,64,64] → Decode_conv_in_shortcut → [B,512,64,64]
             → 4 次 upsample → image
```

这张图的核心结论：**新 VAE 和 DA-VAE 16× 在 1K 时 DiT token 数相同，但每个 token 的输入通道容量不同；DA-VAE 的优势不是 token 更少，而是保留了 base/detail 结构和预训练模型兼容性。**
